...
- deduplications of ADR data have been attempted e.g. for fatigue, malaise, lethary or asthenia, I've decided to use fatigue only to cover all of these similar-meaning terms; the only exception is when the same term is reported as an ADR and also during postmarketing period, then both terms are kept (note: there may still be some duplicated terms that I've missed...)
...
*May turn each drug into sub-headings later for quick access from table of contents in blog*
