...
- there are 2 types: [`nn.Dropout()`](https://pytorch.org/docs/stable/generated/torch.nn.Dropout.html) with an [example](https://machinelearningmastery.com/using-dropout-regularization-in-pytorch-models/) and [`F.dropout()`](https://pytorch.org/docs/stable/generated/torch.nn.functional.dropout.html) (note: F = functional)
...
The code below may need more tweaking.
...
The code output for one of the runs (note: epoch 0 is first epoch):
