...
*This type of work may work better if focussing on a specific therapeutic target, and trying to learn the patterns of ADRs for the same target. However, the bigger picture here is trying to formulate a CYP450-ADRs drug profile first.*
...
* Drug molecular representations e.g. SMILES, SMARTS etc. from other sources e.g. PubChem or ChEMBL or others

*Mermaid chart to show flow of ideas (need to transcribe from my scribbles to this doc)*
...
Plans for posts:
First post will focus on getting/compiling/curating some data from well-established and reputable drug reference texts and somehow linking it with CYP3A4/5 substrates (this may have future potentials to link back to the previous work on CYP450 and drug molecules to fill in the missing jigsaw puzzles). 

While trying to think how I'm going to approach the first post, there will also be variations in "assays in humans" (as people are familiar with in vitro experimental errors or variations, it'll be the same for the in vivo ones) where one of the first aspects I can think of is pharmacogenomics - variations of drug responses due to human genome differences (*check definitions*), and also clinical trials are usually designed in very restricted, highly selective and controlled environments so they won't be very representative of the real patient populations - this needs to be taken into account when using med data from data sheets (postmarketing data on the other hand will be very valuable as more reflective of real-life uses). There are likely other factors that are not mentioned here.

I think the very first post may take the longest time as I've never compiled data of this kind, but will give it a shot and see how feasible it may be (it may take several smaller posts to complete as it may be time-consuming or more difficult than expected).
